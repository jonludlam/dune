...
let build_package_maps packages_with_files ~opam_prefix =
  List.fold_left
    ...
    ~f:(fun (file_to_pkg, installed, mlds, configs) (pkg_name, files) ->
      ...
      in
      let mlds =
        if List.is_empty mld_files then mlds else Package.Name.Map.set mlds pkg_name mld_files
      in
...
let create_impl context =
  ...
  | Some prefix ->
    ...
    Log.info [ Pp.textf "Package_discovery.create_impl: processing %d libs" (List.length all_libs) ];
    let lib_mappings =
      build_mappings_from_changes_data ~file_to_package_map:file_to_package all_libs
    in
    let obc_libs = Package.Name.Map.find lib_mappings.libs_of_package (Package.Name.of_string "ocaml-base-compiler") in
    Log.info
      [ Pp.textf
