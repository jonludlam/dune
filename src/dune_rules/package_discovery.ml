...
let extract_lib_names_from_meta ~base_name (meta : Meta.Simplified.t) =
  let rec loop ~full_name acc (meta : Meta.Simplified.t) =
    ...
    List.fold_left
      meta.Meta.Simplified.subs
      ~init:acc
      ~f:(fun acc (sub : Meta.Simplified.t) ->
        let sub_full_name =
...
let build_file_to_package_map packages_with_files ~opam_prefix =
  List.fold_left packages_with_files ~init:Path.Map.empty ~f:(fun acc (pkg_name, files) ->
...
let build_installed_files_map packages_with_files =
  List.fold_left
    ...
    ~f:(fun acc (pkg_name, files) -> Package.Name.Map.set acc pkg_name files)
;;
    ...
    ~f:(fun acc (pkg_name, files) ->
      let mld_files =
        List.filter_map files ~f:(fun file_str ->
          ...
          match parts with
          | "doc" :: pkg :: "odoc-pages" :: _
            when String.equal pkg (Package.Name.to_string pkg_name) ->
            if String.is_suffix file_str ~suffix:".mld"
            then Some (Path.relative opam_prefix file_str)
            else None
          | _ -> None)
      in
      if List.is_empty mld_files then acc else Package.Name.Map.set acc pkg_name mld_files)
;;
    ...
    ~f:(fun acc (pkg_name, files) ->
      let config_file_opt =
        List.find_opt files ~f:(fun file_str ->
          let parts = String.split file_str ~on:'/' in
          match parts with
          | [ "doc"; pkg; "odoc-config.sexp" ]
            when String.equal pkg (Package.Name.to_string pkg_name) -> true
          | _ -> false)
      in
...
let create_impl context =
  ...
  | Some prefix ->
    ...
    Package.Name.Map.iteri mlds_map ~f:(fun pkg mlds ->
      if not (List.is_empty mlds)
      then
        Log.info
          [ Pp.textf
              "Package_discovery: Package %s has %d mld files"
              (Package.Name.to_string pkg)
              (List.length mlds)
          ]);
    (* Debug: Log discovered config files *)
    Package.Name.Map.iteri config_map ~f:(fun pkg config ->
      if not Odoc_config.(config.deps.libraries = [] && config.deps.packages = [])
      then
        Log.info
          [ Pp.textf
              "Package_discovery: Package %s has odoc-config with %d libs, %d pkgs"
              (Package.Name.to_string pkg)
    ...
    let* lib_mappings_complete =
          ...
          | Some lib ->
            ...
            then Memo.return []
              ...
              let* meta =
                Meta.load
                  meta_path_external
                  ~name:(Some (Package.Name.of_string (Lib_name.to_string lib_name)))
              in
      ...
      List.fold_left
        all_additional
        ~init:lib_mappings
        ~f:(fun acc (lib_name, lib, pkg_name) ->
          (* Only add if not already present *)
          if Lib_name.Map.mem acc.package_of_lib lib_name
          then acc
          else
            { package_of_lib = Lib_name.Map.set acc.package_of_lib lib_name pkg_name
            ; libs_of_package =
...
let package_of_lib_name t lib_name = Lib_name.Map.find t.package_of_lib lib_name
     ...
     | Some pkg ->
       ...
       (match Package.Name.Map.find t.installed_files pkg with
        ...
        | Some files ->
          ...
          let rel_dir =
            if String.is_prefix src_dir_str ~prefix:prefix_str
            then (
              let prefix_len = String.length prefix_str in
              let src_len = String.length src_dir_str in
              if prefix_len < src_len && src_dir_str.[prefix_len] = '/'
              then
                String.sub
                  src_dir_str
                  ~pos:(prefix_len + 1)
                  ~len:(src_len - prefix_len - 1)
              else if prefix_len = src_len
              then ""
              else src_dir_str)
            else src_dir_str
          in
          ...
          let cmti_rel =
            ...
            then module_name_lower ^ ".cmti"
            else rel_dir ^ "/" ^ module_name_lower ^ ".cmti"
          in
          let cmt_rel =
            if String.is_empty rel_dir
            then module_name_lower ^ ".cmt"
            else rel_dir ^ "/" ^ module_name_lower ^ ".cmt"
          in
          if List.mem files cmti_rel ~equal:String.equal
          then Some (Path.relative src_dir (module_name_lower ^ ".cmti"))
          else if List.mem files cmt_rel ~equal:String.equal
          then Some (Path.relative src_dir (module_name_lower ^ ".cmt"))
          else None))
;;
        ...
        | Some files ->
          ...
          let module_names =
            List.filter_map files ~f:(fun file_rel ->
              ...
              let matches_dir =
                if String.is_empty rel_dir
                then not (String.contains file_rel '/')
                else String.is_prefix file_rel ~prefix:(rel_dir ^ "/")
              in
              if matches_dir
              then (
                (* Extract basename *)
                ...
                if String.is_suffix basename ~suffix:".cmti"
                then (
                  match String.drop_suffix basename ~suffix:".cmti" with
                  | Some module_name -> Some (String.capitalize_ascii module_name)
                  | None -> None)
                else if
                  String.is_suffix basename ~suffix:".cmt"
                  && not (String.is_suffix basename ~suffix:".cmti")
                then (
...
(* Get the version of an installed package by looking at .opam-switch/packages/ directory *)
let version_of_package t pkg =
  match t.opam_prefix with
  | None -> Memo.return None
  | Some prefix ->
    let packages_dir = Path.relative prefix ".opam-switch/packages" in
    let packages_dir_external = Path.as_outside_build_dir_exn packages_dir in
    let* dir_result = Fs_memo.dir_contents ~force_update:false packages_dir_external in
    (match dir_result with
     | Error _ -> Memo.return None
     | Ok contents ->
       let pkg_name = Package.Name.to_string pkg in
       (* Look for directories matching {pkg_name}.{version} *)
       let matching_dirs =
         Fs_cache.Dir_contents.to_list contents
         |> List.filter_map ~f:(fun (dirname, _kind) ->
           (* Check if this matches the pattern {pkg_name}.{version} *)
           match String.lsplit2 dirname ~on:'.' with
           | Some (name, version) when String.equal name pkg_name -> Some version
           | _ -> None)
       in
       (* Return the first matching version (there should only be one) *)
       Memo.return (List.hd_opt matching_dirs))
;;
...
module For_tests = struct
  ...
  let find_ocaml_packages ~opam_root =
    ...
    if not (Sys.file_exists (Path.to_string install_dir))
    then []
    else (
      let changes_files =
        ...
        | files ->
          ...
          |> List.filter_map ~f:(fun filename ->
            ...
            then (
              match String.drop_suffix filename ~suffix:".changes" with
              | Some pkg_name ->
                if String.is_prefix pkg_name ~prefix:"ocaml"
                then Some (Package.Name.of_string pkg_name)
                else None
              | None -> None)
            else None)
        | exception _ -> []
  ...
  let read_real_changes_file ~opam_root ~package_name =
    let changes_file =
      Path.relative
        ...
        (sprintf ".opam-switch/install/%s.changes" (Package.Name.to_string package_name))
    in
    if not (Sys.file_exists (Path.to_string changes_file))
    then None
    else (
      try
  ...
  let find_archive_in_opam ~opam_root ~filename =
    ...
    let rec search_dir dir =
      if not (Sys.file_exists (Path.to_string dir))
      then None
      else (
        try
          ...
          Array.fold_left entries ~init:None ~f:(fun acc entry ->
            ...
            | None ->
              ...
              if String.equal entry filename
              then Some entry_path
              else if Sys.is_directory (Path.to_string entry_path)
              then search_dir entry_path
              else None)
        with
        | _ -> None)
    in
