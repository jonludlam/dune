...
let get_lib_paths ctx ~stdlib_opt requires pkg_discovery =
  ...
  List.filter_map libs ~f:(fun lib ->
    ...
    | None ->
      (* Installed library *)
      let lib_pkg_opt = Package_discovery.package_of_library pkg_discovery lib in
      Option.map lib_pkg_opt ~f:(fun lib_pkg -> lib, Paths.odocs ctx (Target.Lib (lib_pkg, lib)))
    | Some local_lib ->
      (* Local library *)
      let lib_t = Lib.Local.to_lib local_lib in
...
let link_odoc_rules sctx (odoc_file : Artifact.t) ~pkg ~requires =
  ...
  let run_odoc =
    run_odoc
      ...
      ; odoc_pkg_flags ctx pkg_discovery ~current_pkg_opt:(Artifact.pkg odoc_file) ~artifact_config
...
let compute_link_requires ~artifact =
  ...
  then Memo.return base_requires
    ...
    Memo.return
      (Resolve.bind base_requires ~f:(fun base_libs -> Resolve.return (base_libs @ extra_libs)))
;;
...
let setup_toplevel_index_mld sctx ~mode =
  ...
  | Some content ->
    ...
    add_rule sctx (Action_builder.write_file (Path.as_in_build_dir_exn output_path) content)
  | None -> Memo.return ()
;;
...
let handle_sidebar_root sctx ~dir ~mode =
  ...
  let private_lib_subdirs =
    List.map private_local_libs ~f:(fun local_lib ->
      Odoc_scope.lib_unique_name local_lib)
  in
  ...
  Memo.return
    (Build_config.Gen_rules.make
       ~build_dir_only_sub_dirs:
         (Build_config.Gen_rules.Build_only_sub_dirs.singleton ~dir (Subdir_set.of_list all_subdirs))
       rules)
;;
        ...
        (Rules.collect_unit (fun () ->
           ...
           let* () =
             Memo.parallel_iter all_artifacts ~f:(fun artifact ->
               compile_artifact sctx ~artifact ~lib_artifacts_by_module ~package_lib_names:all_lib_names)
           in
           let* () =
             Memo.parallel_iter all_artifacts ~f:(fun artifact ->
           ...
           in
           let lib_names_with_artifacts =
             List.filter_map all_artifacts ~f:(fun a -> Option.map (Artifact.lib a) ~f:Lib.name)
             |> Lib_name.Set.of_list
           in
           let* lib_alias_dirs =
             Lib_name.Set.to_list all_lib_names
    ...
    ~f:(fun ~ctx ~scope_id ~all_artifacts ~all_lib_names ->
      Memo.return
        (Rules.collect_unit (fun () ->
           ...
           in
           let* () =
             Memo.parallel_iter visible_artifacts ~f:(fun artifact -> link_artifact sctx ~artifact)
           in
           let visible_lib_artifacts =
             List.filter visible_artifacts ~f:(fun a ->
               match Artifact.get_kind a with
...
let gen_project_rules sctx project =
  ...
  |> Memo.parallel_iter_seq ~f:(fun (_, (pkg : Package.t)) ->
    ...
    let should_build =
      Only_packages.mem_all mask || 
        Only_packages.mem mask (Package.name pkg)
    in
    if should_build
    then
...
let setup_private_library_doc_alias sctx ~scope ~dir (l : Library.t) =
  ...
  | Private _ ->
    ...
    let alias_dep alias = Action_builder.deps (Dune_engine.Dep.Set.singleton (Dune_engine.Dep.alias alias)) in
    Rules.Produce.Alias.add_deps (Alias.make ~dir Alias0.private_doc) (alias_dep html_alias_local)
    >>> Rules.Produce.Alias.add_deps (Alias.make ~dir Alias0.doc_full) (alias_dep html_alias_full)
;;
...
let gen_rules sctx ~dir rest =
  ...
  | [ "_html" ] ->
      (* Root HTML directory - allows any package subdirectory *)
      let ctx = Super_context.context sctx in
      let directory_targets =
    ...
    | [ "_mlds"; pkg_name ] ->
      ...
      let lib_subdirs = List.map all_libs ~f:(fun lib -> Lib.name lib |> Lib_name.to_string) in
    ...
    | [ "_html_full" ] ->
      (* Root HTML_full directory - allows any package subdirectory *)
      let ctx = Super_context.context sctx in
      let directory_targets =
