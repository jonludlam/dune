...
let toplevel_index_artifact ctx ~mode =
  ...
  let custom_index =
    ...
    | Some project ->
      (* Access documentation through Package_info which parses the toplevel documentation stanza *)
      let info = Dune_project.info project in
      let doc = Package_info.documentation info in
      doc.index
    | None -> None
  in
  let* source =
...
let discover_installed_lib_artifacts _sctx ctx ~pkg ~lib_name ~lib
  ...
  Log.info
    [ Pp.textf
        "DEBUG: discover_installed_lib_artifacts called for pkg=%s lib=%s"
        (Package.Name.to_string pkg)
        (Lib_name.to_string lib_name)
    ];
...
let discover_installed_pkg_artifacts sctx ctx ~pkg
  ...
  Log.info
    [ Pp.textf
        "DEBUG: discover_installed_pkg_artifacts called for pkg=%s"
        (Package.Name.to_string pkg)
    ];
  let* pkg_discovery = Package_discovery.create ~context:ctx in
