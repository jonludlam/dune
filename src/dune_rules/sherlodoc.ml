  ...
  Command.Args.Dyn
    (let open Action_builder.O in
     ...
     then (
       ...
       in
       (* Compute relative paths from html_root (since URIs are relative to -o argument) *)
       let search_db_uri = Path.reach (Path.build search_db) ~from:(Path.build html_root) in
