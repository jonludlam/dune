...
let test_parse_changes_file () =
  ...
  let changes_file_path =
    Path.outside_build_dir (Path.Outside_build_dir.of_string "/tmp/test.changes")
  in
  (* Test with invalid content - should return empty list *)
  let files =
    Package_discovery.For_tests.parse_changes_file ~changes_file_path "invalid content"
  in
  (* Should gracefully handle invalid content *)
  Printf.printf
    "✓ Parse function handles invalid content gracefully (%d files)\n"
    (List.length files);
...
let test_stdlib_package_discovery () =
  ...
  let opam_prefix =
    ...
    | Some root -> root
    | None -> Path.outside_build_dir (Path.Outside_build_dir.of_string "/tmp/fake_opam")
  in
  (* We need actual Lib.t values, but for this test we use an empty list
     since we're testing the file-to-package mapping *)
  let libs = [] in
  let discovery =
    Package_discovery.For_tests.build_from_changes_data
      ~packages_with_files
      ~opam_prefix
      ~libs
  in
...
let test_ocaml_package_patterns () =
  ...
  let ocaml_packages =
    [ "ocaml-base-compiler.4.14.0"; "ocaml-system.4.14.0"; "ocaml-variants.4.14.0+fp" ]
  in
...
let test_stdlib_archive_ownership () =
  ...
  let test_archive_ownership file_name =
    ...
    let owner =
      Package_discovery.For_tests.package_of_archive_file
        ~packages_with_files
        ~opam_prefix
...
let test_real_stdlib_archives () =
  ...
  | Some opam_root ->
    ...
    (match
       Package_discovery.For_tests.find_archive_in_opam ~opam_root ~filename:"stdlib.cma"
     with
     | None -> Printf.printf "⚠ stdlib.cma not found in opam installation\n"
     | Some stdlib_path ->
       Printf.printf "✓ Found stdlib.cma: %s\n" (Path.to_string stdlib_path);
       (* Try to determine its package *)
       (match
          Package_discovery.For_tests.package_of_real_archive ~archive_path:stdlib_path
        with
        | None -> Printf.printf "⚠ Could not determine package for stdlib.cma\n"
        | Some pkg ->
          let pkg_name = Dune_lang.Package.Name.to_string pkg in
          Printf.printf "✓ stdlib.cma belongs to: %s\n" pkg_name;
          if String.is_prefix pkg_name ~prefix:"ocaml"
          then Printf.printf "✓ Package name starts with 'ocaml' as expected\n"
          else Printf.printf "⚠ Package name does not start with 'ocaml': %s\n" pkg_name));
    (* Try to find unix.cma *)
    (match
       Package_discovery.For_tests.find_archive_in_opam ~opam_root ~filename:"unix.cma"
     with
     | None -> Printf.printf "⚠ unix.cma not found in opam installation\n"
     | Some unix_path ->
       Printf.printf "✓ Found unix.cma: %s\n" (Path.to_string unix_path);
       (* Try to determine its package *)
       (match
          Package_discovery.For_tests.package_of_real_archive ~archive_path:unix_path
        with
        | None -> Printf.printf "⚠ Could not determine package for unix.cma\n"
        | Some pkg ->
          let pkg_name = Dune_lang.Package.Name.to_string pkg in
...
let test_debug_changes_parsing () =
  ...
  | Some opam_root ->
    ...
    (match ocaml_packages with
     ...
     | first_pkg :: _ ->
       Printf.printf
         "Testing changes file parsing for: %s\n"
         (Dune_lang.Package.Name.to_string first_pkg);
       (match
          Package_discovery.For_tests.read_real_changes_file
            ~opam_root
            ~package_name:first_pkg
        with
        | None ->
          Printf.printf
            "⚠ Could not read changes file for %s\n"
            (Dune_lang.Package.Name.to_string first_pkg)
        | Some files ->
          Printf.printf
            "✓ Successfully parsed changes file, found %d files\n"
            (List.length files);
...
let test_multiple_packages () =
  ...
  let packages_with_files =
    [ ocaml_pkg, [ "lib/ocaml/stdlib.cma"; "lib/ocaml/unix.cma" ]
    ; lwt_pkg, [ "lib/lwt/lwt.cma"; "lib/lwt/lwt_unix.cma" ]
    ]
  in
  let opam_prefix =
    match Package_discovery.For_tests.find_opam_root () with
