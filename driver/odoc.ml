...
let compile ~output_dir ~input_file:file ~includes ~warnings_tag ~parent_id ~ignore_output
  =
  let open Cmd in
  let includes =
    Fpath.Set.fold (fun path acc -> Cmd.(acc % "-I" % p path)) includes Cmd.empty
  in
  let output_file =
  ...
  let log = if ignore_output then None else Some (`Compile, Fpath.to_string file) in
...
let compile_md ~output_dir ~input_file:file ~parent_id =
  ...
  let cmd =
    match Id.to_string parent_id with
    | "" -> cmd
    | x -> cmd % "--parent-id" % x
  in
  let desc = Printf.sprintf "Compiling Markdown %s" (Fpath.to_string file) in
  let _lines =
    Cmd_outputs.submit (Some (`Compile, Fpath.to_string file)) desc cmd output_file
  in
  ()
;;
...
let compile_asset ~output_dir ~name ~parent_id =
  let open Cmd in
  let output_file =
    Some Fpath.(output_dir // Id.to_fpath parent_id / ("asset-" ^ name ^ ".odoc"))
  in
  let cmd = !odoc % "compile-asset" % "--name" % name % "--output-dir" % p output_dir in
  let cmd = cmd % "--parent-id" % Id.to_string parent_id in
...
let compile_impl ~output_dir ~input_file:file ~includes ~parent_id ~source_id =
  ...
  let output_file =
    ...
    Some
      Fpath.(
        output_dir // Id.to_fpath parent_id / ("impl-" ^ to_string (set_ext "odoc" f)))
  in
  let cmd = cmd % "--parent-id" % Id.to_string parent_id in
  let cmd = cmd % "--source-id" % Id.to_string source_id in
  let desc = Printf.sprintf "Compiling implementation %s" (Fpath.to_string file) in
  ignore
  @@ Cmd_outputs.submit (Some (`Compile, Fpath.to_string file)) desc cmd output_file
;;
...
let link
  ...
  let output_file =
    ...
    | Some f -> f
    | None -> Fpath.set_ext "odocl" file
  in
  let docs = doc_args docs in
  let libs = lib_args libs in
  let includes = List.fold_left (fun acc i -> acc % "-I" % p i) empty includes in
  let current_package =
  ...
  let cmd = List.fold_left (fun acc k -> acc % "--warnings-tags" % k) cmd warnings_tags in
  let desc = Printf.sprintf "Linking %s" (Fpath.to_string file) in
  let cmd = if custom_layout then cmd % "--custom-layout" else cmd in
  let log = if ignore_output then None else Some (`Link, Fpath.to_string file) in
...
let compile_index
      ...
      ()
  =
  let roots = List.fold_left (fun c r -> Cmd.(c % "--root" % p r)) Cmd.empty roots in
  ...
  let cmd =
    Cmd.(
      ...
      %% occ)
  in
  let desc = Printf.sprintf "Generating index for %s" (Fpath.to_string output_file) in
  let log = if ignore_output then None else Some (`Index, Fpath.to_string output_file) in
  ignore @@ Cmd_outputs.submit log desc cmd (Some output_file)
;;
...
let sidebar_generate ?(ignore_output = false) ~output_file ~json input_file () =
  let json = if json then Cmd.v "--json" else Cmd.empty in
  let cmd =
    Cmd.(!odoc % "sidebar-generate" %% json %% v "-o" % p output_file % p input_file)
  in
  let desc = Printf.sprintf "Generating sidebar for %s" (Fpath.to_string output_file) in
  let log =
...
let html_generate_source
  ...
  let log = if ignore_output then None else Some (`Generate, Fpath.to_string source) in
  ignore @@ Cmd_outputs.submit log desc cmd None
;;
...
let classify dirs =
  ...
  let log = Some (`Classify, String.concat "," (List.map Fpath.to_string dirs)) in
  let lines = Cmd_outputs.submit log desc cmd None |> List.filter (fun l -> l <> "") in
  List.map
    (fun line ->
