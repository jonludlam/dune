...
let read_libraries_from_pkg_defs ~library_name pkg_defs =
  try
    let archive_filename =
      try Some (Fl_metascanner.lookup "archive" [ "byte" ] pkg_defs) with
      | _ ->
        (try Some (Fl_metascanner.lookup "archive" [ "native" ] pkg_defs) with
         | _ -> None)
    in
    let deps =
      try
...
let process_meta_file file =
  ...
  let meta =
    OS.File.with_ic file (fun ic () -> Fl_metascanner.parse ic) () |> Result.get_ok
  in
  let base_library_name =
    if Fpath.basename file = "META"
    then Fpath.parent file |> Fpath.basename
    else Fpath.get_ext file
  in
  let rec extract_name_and_archive
            ~prefix
            ((name, pkg_expr) : string * Fl_metascanner.pkg_expr)
    =
    let library_name = prefix ^ "." ^ name in
    let libraries = read_libraries_from_pkg_defs ~library_name pkg_expr.pkg_defs in
    let child_libraries =
  ...
  let is_not_private (lib : library) =
    not (String.split_on_char '.' lib.name |> List.exists (fun x -> x = "__private__"))
  in
  let libraries =
    libraries
    @ (meta.pkg_children
       ...
       | Some archive_name ->
         ...
         Fpath.Map.update
           ...
           (function
             ...
             | Some y ->
               Logs.err (fun m ->
                 m "Multiple libraries for archive %s: %s and %s." archive_name x.name y);
               Some y)
           acc)
    Fpath.Map.empty
    libraries
