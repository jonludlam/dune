...
let pp_intf fmt (i : intf) =
  Format.fprintf
    fmt
    "@[<hov>{@,mif_hash: %s;@,mif_path: %a;@,mif_deps: %a@,}@]"
    i.mif_hash
    Fpath.pp
    i.mif_path
...
let pp_impl fmt i =
  Format.fprintf
    fmt
    "@[<hov>{@,mip_path: %a;@,mip_src_info: %a;@,mip_deps: %a@,}@]"
    Fpath.pp
    i.mip_path
...
type asset =
  { asset_path : Fpath.t
  ; asset_rel_path : Fpath.t
  }

let pp_asset fmt m =
  Format.fprintf
    fmt
    "@[<hov>{@,asset_path: %a;@,asset_rel_path: %a@,}@]"
    Fpath.pp
    m.asset_path
    Fpath.pp
    m.asset_rel_path
...
let pp fmt t =
  Format.fprintf
    ...
    t.name
    t.version
    (Fmt.Dump.list pp_libty)
    t.libraries
    (Fmt.Dump.list pp_mld)
    t.mlds
    (Fmt.Dump.list pp_asset)
    t.assets
    t.selected
    (Fmt.Dump.list pp_md)
    t.other_docs
...
module Module = struct
  ...
  let vs libsdir cmtidir modules =
    ...
    let mk m_name =
      let exists ext =
        let p = Fpath.(dir // add_ext ext (v (String.uncapitalize_ascii m_name))) in
        let upperP = Fpath.(dir // add_ext ext (v (String.capitalize_ascii m_name))) in
        Logs.debug (fun m -> m "Checking %a (then %a)" Fpath.pp p Fpath.pp upperP);
      ...
      let mk_intf mif_path =
        ...
        | Ok { digest; deps } -> { mif_hash = digest; mif_path; mif_deps = deps }
      ...
      let mk_impl mip_path =
        ...
        let mip_src_info =
          ...
          | None ->
            ...
            None
          | Some src_path ->
            Logs.debug (fun m -> m "Found source file %a for %s" Fpath.pp src_path m_name);
            Some { src_path }
        in
        let mip_deps =
...
module Lib = struct
  let handle_virtual_lib ~dir ~id_override ~lib_name ~all_lib_deps =
    ...
    let lib_deps =
      try Util.StringMap.find lib_name all_lib_deps with
      | _ -> Util.StringSet.empty
    in
  ...
  let v
        ...
        ~id_override
    =
    Logs.debug (fun m -> m "Classifying dir %a for package %s" Fpath.pp dir pkg_name);
    let dirs =
      match cmtidir with
      | None -> [ dir ]
      | Some dir2 -> [ dir; dir2 ]
    ...
    | 0 ->
      (match List.find_opt (fun dir -> List.mem_assoc dir cmi_only_libs) dirs with
       | None -> []
       | Some dir ->
      ...
      List.filter_map
        (fun (archive_name, modules) ->
           ...
           | None ->
             Logs.info (fun m ->
               m
                 "No entry for '%a' in libname_of_archive"
                 Fpath.pp
                 Fpath.(dir / archive_name));
             Logs.info (fun m ->
               m "Unable to determine library of archive %s: Ignoring." archive_name);
             None)
        results
  ;;
...
let mk_mlds docs =
  List.fold_left
    (fun (mlds, assets, others) (doc : Opam.doc_file) ->
       match doc.kind with
       | `Mld ->
         { mld_path = doc.file; mld_rel_path = doc.rel_path } :: mlds, assets, others
       | `Asset ->
         mlds, { asset_path = doc.file; asset_rel_path = doc.rel_path } :: assets, others
       | `Other ->
         mlds, assets, { md_path = doc.file; md_rel_path = doc.rel_path } :: others)
    ([], [], [])
    docs
;;
           ...
           (fun lib ->
              ...
              let new_lib_deps =
                List.fold_left
                       ...
                       (fun hash acc ->
                          ...
                          | deps ->
                            if
                              ...
                              || List.exists (fun d -> Util.StringSet.mem d lib_deps) deps
                            then acc
                  ...
                  lib.modules
              in
              if Util.StringSet.cardinal new_lib_deps > 0
              then
                Logs.debug (fun m ->
                  m
                    "Adding missing deps to %s: %a"
                    lib.lib_name
                    Fmt.(list string)
...
let of_libs ~packages_dir libs =
  let Ocamlfind.Db.{ archives_by_dir; libname_of_archive; cmi_only_libs; all_lib_deps; _ }
    =
...
let of_packages ~packages_dir packages =
  ...
  let deps = if packages = [] then Opam.all_opam_packages () else Opam.deps packages in
  ...
  let ps =
    List.filter_map
      (fun pkg ->
         match
           List.find_opt (fun (pkg', _) -> pkg.Opam.name = pkg'.Opam.name) opam_map
         with
         | None ->
           Logs.warn (fun m -> m "Didn't find package %a in opam_map" Opam.pp pkg);
           None
         | x -> x)
      deps
  in
  let orig =
    List.filter_map
      (fun pkg -> List.find_opt (fun (pkg', _) -> pkg = pkg'.Opam.name) opam_map)
      packages
  in
  let all = orig @ ps in
  let all =
    List.sort_uniq (fun (a, _) (b, _) -> String.compare a.Opam.name b.Opam.name) all
  in
      ...
      (fun (pkg, files) ->
         ...
         let remaps =
           ...
           then []
             ...
             let pkg_path =
               Printf.sprintf "https://ocaml.org/p/%s/%s/doc/" pkg.name pkg.version
             in
             let lib_paths =
               List.map
                 (fun libty ->
                    let lib_name = libty.lib_name in
                    let local_lib_path = Printf.sprintf "%s%s/" local_pkg_path lib_name in
             ...
             { lib with
               modules =
                 ...
                 |> List.map (fun m ->
                   let m_intf =
                     if
                       Util.StringMap.mem m.m_intf.mif_hash duplicate_hashes
                       && Fpath.has_ext "cmt" m.m_intf.mif_path
                     then (
                       match
                         List.filter
                           (fun intf -> Fpath.has_ext "cmti" intf.mif_path)
                           (Util.StringMap.find m.m_intf.mif_hash duplicate_hashes)
                       with
                       | [ x ] -> x
                       | _ -> m.m_intf)
                     else m.m_intf
