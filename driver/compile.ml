...
let marshal (v : partial) filename =
  ...
  Fun.protect ~finally:(fun () -> close_out oc) (fun () -> Marshal.to_channel oc v [])
;;

let find_partials odoc_dir : Odoc_unit.intf Odoc_unit.t list Util.StringMap.t * _ =
        ...
        Fiber.List.map
          (fun (other_unit_name, other_unit_hash) ->
             ...
             | Error _exn ->
               Logs.debug (fun m ->
                 m
                   "Error during compilation of module %s (hash %s, required by %s)"
                   other_unit_name
                   other_unit_hash
    ...
    let rec compile_mod : string -> ('a list, [> `Msg of string ]) Result.t =
      ...
      let map_units =
        Fiber.List.map (fun unit ->
          match
            Hashtbl.find_opt tbl (hash, Odoc.Id.to_string unit.Odoc_unit.parent_id)
          with
          | Some p ->
            Promise.await p;
            None
      ...
      with
      | Not_found -> Error (`Msg ("Module with hash " ^ hash ^ " not found"))
    in
    compile_mod
  in
  ...
  let link : compiled -> linked =
    ...
    let link input_file output_file enable_warnings =
      ...
      Odoc.link
        ...
        ~docs:pages
        ~includes
        ~ignore_output:(not enable_warnings)
        ~warnings_tags
    ...
    then (
      ...
      | `Impl { src_path; _ } ->
        ...
        then (
          ...
          Atomic.incr Stats.stats.generated_units)
      | `Asset ->
        Odoc.html_generate_asset
          ~output_dir
          ~input_file:l.odoc_file
      ...
      | _ ->
        ...
        then (
          ...
          Atomic.incr Stats.stats.generated_units))
  in
  if List.length remaps = 0
  then Fiber.List.iter (html_generate None) linked
  else
