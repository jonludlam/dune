...
let parse_entry (sexp : Sexp.t) =
  ...
  | List (Atom "packages" :: pkgs) -> Some (Ast.Packages (parse_string_list pkgs))
  | _ -> None
;;
...
let load config_file =
  ...
  | Error _ ->
    Logs.err (fun m -> m "Failed to read odoc-config file: %a" Fpath.pp config_file);
    empty
  | Ok s ->
    (try parse s with
     | e ->
       Logs.err (fun m ->
         m
           "Failed to parse config file %a: %s"
           Fpath.pp
           config_file
