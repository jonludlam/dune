    ...
    (fun comp
      ...
      descr ->
       let rec inner (a, b, c, j, d, e, f, i, g, h) =
         ...
         List.iteri (fun i descr -> descr (Atomic.get stats.process_activity.(i))) descr;
         comp (a' - a);
         compimpl (b' - b);
         compmld (c' - c);
         compassets (j' - j);
...
let compute_metric_int prefix suffix description values =
  ...
  | Some (min, max, avg, count) ->
    ...
    ; `Assoc
        ...
        ; "value", `Assoc [ "min", `Int min; "max", `Int max; "avg", `Int avg ]
        ; "units", `String "b"
...
let compute_metric_cmd cmd =
  ...
  | Some (min, max, avg, count) ->
    ...
    ; `Assoc
        ...
        ; "value", `Assoc [ "min", `Float min; "max", `Float max; "avg", `Float avg ]
        ; "units", `String "s"
...
let compute_produced_cmd cmd =
  ...
  compute_metric_int "produced" cmd ("files produced by 'odoc " ^ cmd ^ "'") sizes
;;
...
let compute_longest_cmd cmd =
  ...
  | Some (min, max, avg, _count) ->
    [ `Assoc
        ...
        ; ( "description"
          , `String
              (Printf.sprintf "Time taken by the %d longest calls to 'odoc %s'" k cmd) )
        ; "trend", `String "lower-is-better"
        ]
    ]
;;
  ...
  let result =
    `Assoc
      ...
      ; ( "results"
        , `List
            [ `Assoc
                [ "name", `String "driver.mld"; "metrics", `List (all_metrics html_dir) ]
            ] )
      ]
  in
