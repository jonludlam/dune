module Pkg_args = struct
  ...
  let combine v1 v2 =
    if v1.odoc_dir <> v2.odoc_dir then Fmt.invalid_arg "combine: odoc_dir differs";
    if v1.odocl_dir <> v2.odocl_dir then Fmt.invalid_arg "combine: odocl_dir differs";
  ...
  let pp fmt x =
    let sfp_pp =
      Fmt.(list ~sep:comma (fun fmt (a, b) -> Format.fprintf fmt "(%s, %a)" a Fpath.pp b))
    in
    Format.fprintf
      fmt
      "@[<hov>odoc_dir: %a@;odocl_dir: %a@;includes: %a@;pages: [%a]@;libs: [%a]@]"
      Fpath.pp
      x.odoc_dir
      Fpath.pp
      x.odocl_dir
...
let fix_virtual
  ...
  Logs.debug (fun m ->
    m
      "Fixing virtual libraries: %d precompiled units, %d other units"
       ...
       List.map
         (fun unit ->
            let uhash =
              ...
              | `Intf { hash; _ } -> hash
            in
            if not (Fpath.has_ext "cmt" unit.input_file)
            then unit
            else (
              match Util.StringMap.find uhash all with
              | [ _ ] -> unit
              | xs ->
                let unit_name = Fpath.rem_ext unit.input_file |> Fpath.basename in
                (match
                   List.filter
                     (fun (x : intf t) ->
                        (match x.kind with
                         | `Intf { hash; _ } -> uhash = hash)
                        && Fpath.has_ext "cmti" x.input_file
                        && Fpath.rem_ext x.input_file |> Fpath.basename = unit_name)
                     xs
                 with
                 ...
                 | xs ->
                   Logs.debug (fun m ->
                     m
                       "Duplicate hash found, but multiple (%d) matching cmti found for \
                        %a"
                       (List.length xs)
                       Fpath.pp
                       unit.input_file);
                   let possibles =
                     List.find_map
                       (fun x ->
                          ...
                          | Some x ->
                            if x |> Bos.OS.File.exists |> Result.value ~default:false
                            then Some x
                            else None
                          | None -> None)
                       xs
