...
let get_switch =
  memoize @@ fun () -> Util.lines_of_process Cmd.(opam % "switch" % "show") |> List.hd
;;
...
let pkg_contents { name; _ } =
  ...
  let filename = OpamFile.make @@ OpamFilename.raw @@ Filename.basename changes_file in
  let changed =
    try
      OpamFilename.with_contents
        (fun str ->
           ...
           if OpamStd.String.starts_with ~prefix:"opam-version" str
           then (
...
let deps pkgs =
  let cmd =
    Cmd.(
      ...
      % "--or")
  in
  let cmd = List.fold_left (fun cmd pkg -> Cmd.(cmd % "--required-by" % pkg)) cmd pkgs in
...
let pp_fpath_set fmt set = Fpath.Set.iter (Format.fprintf fmt "%a@." Fpath.pp) set
...
let classify_docs prefix only_package contents =
  ...
  in
  let is_dir f =
    try Sys.is_directory (Fpath.to_string f) with
    | Sys_error _ -> false
  in
  List.fold_left
    (fun acc fpath ->
       ...
       | "doc" :: pkg :: "odoc-pages" :: _ :: _
         ...
         in
         let rel_path =
           Fpath.rem_prefix Fpath.(v "doc" / pkg / "odoc-pages") fpath |> Option.get
         in
       ...
       | "doc" :: pkg :: "odoc-assets" :: _ :: _
         ...
         Logs.debug (fun m -> m "Found odoc page: %a" Fpath.pp fpath);
         let rel_path =
           Fpath.rem_prefix Fpath.(v "doc" / pkg / "odoc-assets") fpath |> Option.get
         in
         let rel_path = Fpath.(v "_assets" // rel_path) in
         { kind = `Asset; file = Fpath.(prefix // fpath); rel_path } :: acc
       | [ "doc"; pkg; _ ] when pkg_match pkg && not (is_dir Fpath.(prefix // fpath)) ->
  ...
  let libs =
    List.fold_left
      (fun set fpath ->
         match Fpath.segs fpath with
         | "lib" :: "stublibs" :: _ -> set
         | "lib" :: pkg :: _ :: _ when Fpath.has_ext ".cmi" fpath && pkg_match pkg ->
        ...
        | Ok contents ->
          ...
          Util.StringSet.fold
            (fun pkg acc ->
               ...
               Logs.debug (fun m -> m "pkg %s Found %d docs" pkg (List.length docs));
               ({ name = pkg; version = "dev" }, { libs; docs; odoc_config }) :: acc)
            packages
            []
        | Error (`Msg msg) ->
...
let check pkgs =
  let cmd =
    Cmd.(opam % "list" % "-i" % "--columns" % "package" % "--color" % "never" % "-s")
  in
...
let pkg_to_dir_map () =
  ...
  let pkg_content =
    List.filter
      (fun (p, _) -> not @@ List.exists (fun (p', _) -> p.name = p'.name) dune_overrides)
      pkg_content
  in
              ...
              Fpath.Map.update
                ...
                (function
                  ...
                  | Some x ->
                    Logs.debug (fun m ->
                      m
                        "Multiple packages (%a,%a) found for dir %a"
                        pp
                        x
                        pp
                        p
