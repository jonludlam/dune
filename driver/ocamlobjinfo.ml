...
let source_possibilities file =
  ...
  in
  let pp =
    if Astring.String.is_suffix ~affix:".pp.ml" file
    then (
...
let get_source file srcdirs =
  ...
  let lines_res = Worker_pool.submit ("Ocamlobjinfo " ^ Fpath.to_string file) cmd None in
  let lines =
    match lines_res with
    | Ok l -> String.split_on_char '\n' l.output
    | Error e ->
      Logs.err (fun m ->
        m "Error finding source for module %a: %s" Fpath.pp file (Printexc.to_string e));
      []
  in
  let f =
    List.filter_map
      (fun line ->
         ...
         then (
           ...
           let possibilities =
             List.map
               (fun dir ->
                  List.map (fun poss -> Fpath.(dir / poss)) (source_possibilities name))
               srcdirs
             |> List.flatten
           in
           List.find_opt
