...
let find_universe_and_version pkg_name =
  Bos.OS.Dir.contents Fpath.(v !prep_path / "universes")
  >>= fun universes ->
  let universe =
    match
      List.find_opt
  ...
  universe
  >>= fun u ->
  Bos.OS.Dir.contents ~rel:true Fpath.(u / pkg_name)
  >>= fun version ->
  ...
  | Ok files ->
    List.fold_left
      (fun acc file ->
         ...
         if Astring.String.is_suffix ~affix:".cma" base
         then (
...
let of_voodoo pkg =
  ...
  let config =
    let config_file = Fpath.(pkg_path / "doc" / pkg.name / "odoc-config.sexp") in
    match Bos.OS.File.read config_file with
    | Error (`Msg msg) ->
      Logs.debug (fun m -> m "No config file found: %a\n%s\n%!" Fpath.pp config_file msg);
      Global_config.empty
    | Ok s ->
  ...
  let meta_libraries : Packages.libty list =
    ...
    |> List.filter_map (fun meta_file ->
      ...
      Some
        (List.concat_map
           (fun directory ->
              Logs.debug (fun m -> m "Processing directory: %a\n%!" Fpath.pp directory);
              Packages.Lib.v
                ~libname_of_archive
                ~pkg_name:pkg.name
                ~dir:directory
        ...
        (fun (cur_opt, acc) file ->
           ...
           | "prep" :: "universes" :: u :: p :: v :: (_ :: _ as rest) when p = pkg_name ->
             ...
             (match cur_opt with
              ...
              | _ ->
                ( Some { name = p; version = v; universe = u; blessed; files = [ file ] }
                , cur_opt :: acc ))
           | _ -> cur_opt, acc)
        (None, [])
        sorted
...
let extra_paths compile_dir =
  let contents =
    Bos.OS.Dir.fold_contents ~dotfiles:true (fun p acc -> p :: acc) [] compile_dir
  in
  ...
  let pkgs, libs, libs_of_pkg =
    match contents with
    | Error _ -> Util.StringMap.empty, Util.StringMap.empty, Util.StringMap.empty
    | Ok c ->
      List.fold_left
        (fun (pkgs, libs, libs_of_pkg) abs_path ->
           ...
           | [ "p"; pkg; _version; "doc"; l ] when l = pkg_marker ->
             ...
             Util.StringMap.add pkg (Fpath.parent path) pkgs, libs, libs_of_pkg
           | [ "u"; _universe; pkg; _version; "doc"; libname; l ] when l = lib_marker ->
             Logs.debug (fun m -> m "Found lib marker: %a" Fpath.pp path);
             ( pkgs
             , Util.StringMap.add libname (Fpath.parent path) libs
             , add_libs pkg libname libs_of_pkg )
...
let write_lib_markers odoc_dir pkgs =
  let write file str =
    ...
    | Ok () -> ()
    | Error (`Msg msg) -> Logs.err (fun m -> m "Failed to write lib marker: %s" msg)
  in
  List.iter
    (fun (pkg : Packages.t) ->
       ...
       write
         marker
         (Fmt.str
            "This marks this directory as the location of odoc files for the package %s"
            pkg.name);
       List.iter
         (fun (lib : Packages.libty) ->
            ...
            write
              marker
              (Fmt.str
                 "This marks this directory as the location of odoc files for library %s \
                  in package %s"
                 lib.lib_name
                 pkg.name))
         libs)
