...
let make_index ~dirs ~rel_dir ~libs ~pkgs ~index ~enable_warnings ~content
  ...
  let pages = List.map (fun pkg -> pkg.Packages.name, Odoc_unit.doc_dir pkg) pkgs in
  let libs =
    List.map (fun (pkg, lib) -> lib.Packages.lib_name, Odoc_unit.lib_dir pkg lib) libs
  in
...
let module_list ppf lib =
  ...
  | [] -> fpf ppf "No module."
  | _ :: _ ->
    let modules = List.sort (fun m m' -> String.compare m.m_name m'.m_name) modules in
    fpf ppf "{!modules:";
...
let package ~dirs ~pkg ~index =
  let library_list ppf pkg =
    let print_lib lib = fpf ppf "{2 Library %s}@\n%a@\n" lib.lib_name module_list lib in
    let libraries =
      List.sort (fun lib lib' -> String.compare lib.lib_name lib'.lib_name) pkg.libraries
    in
  ...
  let content pkg ppf =
    ...
    List.iter
      (fun { mld_rel_path; _ } ->
         ...
         fpf ppf "@\n{!/%s/%s}@\n" pkg.name page)
      pkg.mlds;
    if not (List.is_empty pkg.libraries)
    then fpf ppf "{1 API}@\n@\n%a@\n" library_list pkg
  in
...
let src ~dirs ~pkg ~index =
  let content ppf =
    ...
    fpf
      ppf
      "{0 Sources}@\n\
       This contains the rendered source for [%s]. Use the sidebar to navigate them."
      pkg.name
  in
...
let package_list ~dirs ~remap all =
  let content all ppf =
    ...
    let print_pkg pkg =
      if pkg.selected || not remap
      then fpf ppf "- {{!/%s/page-index}%s}@\n" pkg.name pkg.name
    in
    List.iter print_pkg sorted_packages
  in
  let content = content all in
  let rel_dir = Fpath.v "./" in
  make_index ~dirs ~rel_dir ~pkgs:all ~libs:[] ~index:None ~content ~enable_warnings:true
;;
...
let content dir _pkg libs _src subdirs all_libs pfp =
  ...
  then (
    ...
    Fpath.Set.iter
      (fun x ->
         if Fpath.basename x <> "opam_switch" then fpf pfp "%s/ " (Fpath.basename x))
      subdirs;
    fpf pfp "opam_switch\n%!")
  else fpf pfp "@short_title %s\n" (Fpath.basename dir);
  if Fpath.Set.cardinal subdirs > 0
  then (
    ...
    Fpath.Set.iter
      ...
      subdirs);
  if (not is_root) && List.length libs > 0
  then
    List.iter
      (fun (_, lib) ->
...
let make_custom dirs index_of (pkg : Packages.t) : Odoc_unit.mld Odoc_unit.t list =
  let pkgs = [ pkg ] in
  let pkg_dirs =
    List.fold_right
      (fun pkg dirs -> Fpath.Map.add (Fpath.to_dir_path pkg.Packages.pkg_dir) pkg dirs)
      pkgs
      Fpath.Map.empty
  in
  let lib_dirs =
  ...
  let pkg_src_dirs =
    List.fold_left
      (fun acc pkg -> Fpath.Map.add (Odoc_unit.src_dir pkg |> Fpath.to_dir_path) pkg acc)
      Fpath.Map.empty
      pkgs
  in
  let all_dirs =
    Fpath.Set.union
      (Fpath.Map.dom pkg_dirs)
  ...
  let rec all_parents path =
    ...
    if Fpath.compare parent (Fpath.v "./") = 0 || Fpath.compare parent (Fpath.v "/") = 0
    then [ path ]
  ...
  let all_indexes =
    List.fold_right
      (fun pkg acc ->
         ...
         let indexes =
           List.filter (fun mld -> Fpath.basename mld.mld_rel_path = "index.mld") mlds
         in
         let index_paths =
           List.map (fun mld -> Fpath.(pkg.pkg_dir // mld.mld_rel_path |> parent)) indexes
           |> Fpath.Set.of_list
         in
  ...
  Fpath.Set.fold
    (fun p acc ->
       ...
       then (
         ...
         Logs.debug (fun x ->
           x
             "dir: %a pkg: %a lib: %a src: %a pkg_src: %a subdirs: %a"
             Fpath.pp
             p
             Fmt.string
             pkg.Packages.name
         ...
         Logs.debug (fun m ->
           m "pkgs: %a" Fmt.Dump.(list string) (List.map (fun p -> p.Packages.name) pkgs));
         let idx =
           make_index
             ~dirs
             ~rel_dir:p
