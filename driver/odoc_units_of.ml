...
let packages ~dirs ~extra_paths ~remap ~indices_style (pkgs : Packages.t list) : any list =
  ...
  let libs_of_pkg =
    ...
    in
    List.fold_left
      (fun acc pkg -> Util.StringMap.add pkg.Packages.name (libs_of_pkg pkg) acc)
      extra_libs_of_pkg
      pkgs
  in
  ...
  let args_of =
    ...
    fun pkg lib_deps : Pkg_args.t ->
      ...
      | None ->
        let result =
          Pkg_args.combine (base_args pkg lib_deps) (args_of_config pkg.Packages.config)
        in
  ...
  let index_of pkg =
    ...
    in
    { roots; output_file; json = false; search_dir = doc_dir pkg; sidebar = Some sidebar }
  in
  let make_unit
        ~name
        ~kind
    ...
    let odoc_file = odoc_dir // rel_dir / (String.uncapitalize_ascii name ^ ".odoc") in
    (* odoc will uncapitalise the output filename *)
    let odocl_file = odocl_dir // rel_dir / (String.uncapitalize_ascii name ^ ".odocl") in
    let input_copy =
      if stash_input
      then Some (odoc_dir // rel_dir / (String.uncapitalize_ascii name ^ ".cmti"))
      else None
    in
    { output_dir = odoc_dir
  ...
  in
  let of_intf hidden pkg (lib : Packages.libty) lib_deps (intf : Packages.intf) : intf t =
    ...
    make_unit
      ...
      ~kind
      ~rel_dir
      ~input_file:intf.mif_path
      ~pkg
      ~lib_deps
      ~enable_warnings:pkg.selected
  ...
  let of_impl pkg lib lib_deps (impl : Packages.impl) : impl t option =
    ...
    | Some { src_path } ->
      ...
      let kind =
        ...
        let src_id = Fpath.(src_lib_dir pkg lib / src_name) |> Odoc.Id.of_fpath in
      ...
      Some unit
  in
  let of_module pkg (lib : Packages.libty) lib_deps (m : Packages.modulety) : any list =
  ...
  let of_md pkg (md : Packages.md) : md t list =
    ...
    | ".md" ->
      ...
      let rel_dir = doc_dir pkg // Fpath.parent md_rel_path |> Fpath.normalize in
      let kind = `Md in
      let name = md_path |> Fpath.rem_ext |> Fpath.basename |> ( ^ ) "page-" in
      ...
      [ unit ]
    | _ ->
      Logs.debug (fun m -> m "Skipping non-markdown doc file %a" Fpath.pp md.md_path);
      []
  in
  let of_asset pkg (asset : Packages.asset) : asset t list =
    let open Fpath in
    let { Packages.asset_path; asset_rel_path } = asset in
    let rel_dir = doc_dir pkg // Fpath.parent asset_rel_path |> Fpath.normalize in
    let kind = `Asset in
    let unit =
  ...
  let of_package (pkg : Packages.t) : any list =
    ...
    let pkg_index () :> any list =
      let has_index_page =
        List.exists
          ...
          pkg.mlds
      in
      if has_index_page || (remap && not pkg.selected)
      then []
      else (
        let index = index_of pkg in
    ...
    | Automatic when pkg.name = Monorepo_style.monorepo_pkg_name ->
      let others :> any list =
        Landing_pages.make_custom
          dirs
          index_of
          (List.find (fun p -> p.Packages.name = Monorepo_style.monorepo_pkg_name) pkgs)
      in
