  ...
  | _ ->
    (match Opam.check packages with
     ...
     | Error missing ->
       Logs.err (fun m ->
         m
           "Error: Unknown/uninstalled packages: %a"
...
let run_inner
  ...
  let index_mld_content =
    Option.bind index_mld (fun fpath ->
      ...
      | Error (`Msg msg) ->
        Logs.err (fun m -> m "Failed to read index_mld file '%a': %s" Fpath.pp fpath msg);
        exit 1)
  in
  Eio_main.run
  @@ fun env ->
  ...
  let () =
    Eio.Fiber.both
      (fun () ->
         ...
         in
         Logs.debug (fun m ->
           m "odoc_dirs: %a" (Fmt.Dump.list Fpath.pp) (Fpath.Set.to_list odoc_dirs));
         let occurrence_file =
           let output = Fpath.( / ) odocl_dir "occurrences-all.odoc-occurrences" in
           let () = Odoc.count_occurrences ~input:(Fpath.Set.to_list odoc_dirs) ~output in
           output
         in
         let () =
           Compile.html_generate
             ~occurrence_file
             ~remaps
             ~generate_json
             ~simplified_search_output:false
             html_dir
             linked
         in
...
let compile_grep =
  ...
  Arg.(value & opt (some string) None & info [ "compile-grep" ] ~doc ~docs:Manpage.s_none)
;;

let link_grep =
  let doc = "Show link commands containing the string" in
  Arg.(value & opt (some string) None & info [ "link-grep" ] ~doc ~docs:Manpage.s_none)
;;

let generate_grep =
  let doc = "Show html-generate commands containing the string" in
  Arg.(value & opt (some string) None & info [ "html-grep" ] ~doc ~docs:Manpage.s_none)
;;

let index_grep =
  let doc = "Show compile-index commands containing the string" in
  Arg.(value & opt (some string) None & info [ "index-grep" ] ~doc ~docs:Manpage.s_none)
;;
...
let index_mld =
  let doc =
    "Provide an index.mld file to serve as the top-level index of the documentation"
  in
  Arg.(
    value
...
let cmd =
  let doc =
    "Documents packages present in your opam switch. This mode will select all opam \
     packages that are dependencies of the selected packages."
  in
