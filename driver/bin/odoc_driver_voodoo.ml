...
let generate_status ~html_dir pkg =
  let redirections =
    ...
    let create_redirection old_path new_path =
      if Bos.OS.File.exists old_path |> Result.get_ok
      then ()
      else (
...
let run
  ...
  let all, extra_paths, actions, generate_json, occurrence_file, odocl_dirs =
    ...
    let odocl_dirs =
      List.map (fun l -> Fpath.(odocl_dir // Odoc_unit.lib_dir all l)) all.libraries
    in
    let occurrence_file = Fpath.(odocl_dir // Voodoo.occurrence_file_of_pkg pkg) in
  ...
  let units =
    ...
    Odoc_units_of.packages ~dirs ~indices_style:Voodoo ~extra_paths ~remap:false all
  in
  ...
  let () =
    ...
    | LinkAndGen | All ->
      let linked =
        Compile.link ~warnings_tags:[ package_name ] ~custom_layout:false compiled
      in
      let () = Odoc.count_occurrences ~input:odocl_dirs ~output:occurrence_file in
      let () =
...
let action_conv =
  Arg.enum [ "compile-only", CompileOnly; "link-and-gen", LinkAndGen; "all", All ]
;;

let actions =
  let doc =
    "Actions to perform. Valid values are 'compile-only', 'link-and-gen' and 'all'."
  in
...
let odoc_dir =
  ...
  Arg.(required & opt (some Common_args.fpath_arg) None & info [ "odoc-dir" ] ~doc)
;;

let odocl_dir =
  let doc = "Directory in which the intermediate odocl files go" in
  Arg.(value & opt (some Common_args.fpath_arg) None & info [ "odocl-dir" ] ~doc)
;;
