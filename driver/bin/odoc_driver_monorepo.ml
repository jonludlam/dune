      ...
      (fun () ->
         let units =
           ...
           Odoc_units_of.packages
             ~dirs
             ~indices_style:Odoc_units_of.Automatic
             ~extra_paths
             ~remap:false
             all
         in
         ...
         let occurrence_file =
           let output = Fpath.( / ) odoc_dir "occurrences-all.odoc-occurrences" in
...
let path =
  ...
  Arg.(value & pos 0 Common_args.fpath_arg (Fpath.v ".") & info ~doc ~docv:"PATH" [])
;;
...
let cmd =
  ...
  Cmd.v
    info
    Term.(const run $ A.dirs_term $ path $ extra_pkgs $ extra_libs $ Common_args.term)
;;
