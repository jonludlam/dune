...
let dune_describe dir =
  ...
  match out with
  | Error _ -> []
  | Ok out -> of_dune_describe out.Run.output
;;
...
let of_dune_build dir ~extra_pkgs ~extra_libs =
  ...
  | Ok c ->
    ...
    let rec with_trans_deps =
      ...
      fun lib_name ->
        try Hashtbl.find cache lib_name with
        | Not_found ->
          let libs =
    ...
    in
    let all_lib_deps =
      Util.StringMap.mapi (fun lib_name _ -> with_trans_deps lib_name) all_lib_deps
    in
        ...
        (fun acc item ->
           ...
           | Library lib ->
             ...
             (match libname_opt with
              | Some (libname, _) ->
                let archive = Fpath.(append dir (v lib.source_dir / libname)) in
                Logs.debug (fun m ->
                  m "libname_of_archive: %a -> %s" Fpath.pp archive lib.name);
      ...
      List.filter_map
        (fun lib ->
           ...
           | Some (libname, cmtidir) ->
             ...
             let id_override =
               Fpath.relativize ~root:Fpath.(v "_build/default") Fpath.(v lib.source_dir)
               |> Option.map Fpath.to_string
             in
             (match id_override with
              | None ->
                Logs.warn (fun m ->
                  m "Could not determine id_override for library '%s'" libname)
              | _ -> ());
             if Fpath.Set.mem cmtidir cset
             then
               Some
                 (Packages.Lib.v
                    ~libname_of_archive
    ...
    let assets =
      find_docs ".jpg"
      |> List.map (fun (p, r) -> { Packages.asset_path = p; asset_rel_path = r })
    in
    ...
    let global_config =
      { Global_config.deps =
          { packages = extra_pkgs
          ; libraries =
              extra_libs @ List.map (fun (lib : Packages.libty) -> lib.lib_name) libs
          }
      }
    in
    let local =
    ...
    let global =
      Packages.of_libs
        ...
        (List.map (fun (l : library) -> l.name) global_libs |> Util.StringSet.of_list)
    in
    local @ global
;;
