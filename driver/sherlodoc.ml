...
let index ?(ignore_output = false) ~format ~inputs ~dst ?favored_prefixes () =
  ...
  in
  let log = if ignore_output then None else Some (`Sherlodoc, Fpath.to_string dst) in
...
let js dst =
  ...
  let _lines =
    submit_ignore_failures (Some (`Sherlodoc, Fpath.to_string dst)) desc cmd (Some dst)
  in
  ()
;;
