...
let get_dir lib =
  try
    init ();
    Fl_package_base.query lib |> fun x -> Ok Fpath.(v x.package_dir |> to_dir_path)
  with
  | e ->
...
let archives pkg =
  ...
  let get_1 preds =
    try
      [ Fl_metascanner.lookup "archive" preds package.Fl_package_base.package_defs ]
    with
    | _ -> []
  in
  match pkg with
...
let rec dep =
  ...
  fun pkg ->
    init ();
    try Util.StringMap.find pkg !memo with
    | Not_found ->
      (try
...
module Db = struct
  ...
  let create libs =
    ...
    in
    let all_libs_set = Util.StringSet.union all_libs_deps (Util.StringSet.of_list libs) in
      ...
      List.fold_right
        (fun lib_name acc ->
           ...
           | Error (`Msg msg) ->
             Logs.err (fun m ->
               m
                 "Error finding dependencies of library '%s' through ocamlfind: %s"
                 lib_name
                 msg);
             acc)
        all_libs
        ...
        (fun lib ->
           ...
           | Ok p ->
             ...
             let archives =
               List.map
                 (fun x ->
                    try Filename.chop_extension x with
                    | e ->
    ...
    let libname_of_archive =
      List.fold_left
        (fun map (lib, dir, archives) ->
             ...
             Fpath.Map.update
               ...
               (function
                 ...
                 | Some x ->
                   Logs.info (fun m ->
                     m
                       "Multiple libraries for archive %s: %s and %s. Arbitrarily \
                        picking the latter."
                       archive
                       x
                       lib);
           ...
           | xs ->
             Logs.err (fun m ->
               m "multiple archives detected: [%a]" Fmt.(list ~sep:sp string) xs);
             assert false)
        Fpath.Map.empty
        lib_dirs_and_archives
    in
