...
let verbose =
  ...
  Arg.(value & flag & info [ "v"; "verbose" ] ~doc ~docs:Manpage.s_common_options)
;;
...
let odoc_bin =
  ...
  Arg.(
    value & opt (some string) None & info [ "odoc" ] ~doc ~docs:Manpage.s_common_options)
;;
...
let generate_json =
  ...
  Arg.(value & flag & info [ "json-output" ] ~doc ~docs:Manpage.s_common_options)
;;
...
let with_dirs dirs fn : unit =
  ...
  with_dir odoc_dir "odoc-%s"
  @@ fun odoc_dir () ->
  with_dir odocl_dir "odocl-%s"
  @@ fun odocl_dir () ->
  with_dir index_dir "index-%s"
  @@ fun index_dir () ->
  with_dir mld_dir "mld-%s"
  @@ fun mld_dir () -> fn ~odoc_dir ~odocl_dir ~index_dir ~mld_dir ()
;;
